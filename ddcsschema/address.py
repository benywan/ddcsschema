import ddcsschema
from ddcsschema.base import b58encode, b58decode, validate_b58_checksum
from ddcsschema.hashing import double_sha256, hash160
from ddcsschema.error import InvalidAddress
from ddcsschema.schema import ADDRESS_LENGTH, ADDRESS_PREFIXES, PUBKEY_ADDRESS, SCRIPT_ADDRESS
...
def validate_address_prefix(addr_bytes):
    if ord(addr_bytes[0]) not in ADDRESS_PREFIXES[ddcsschema.BLOCKCHAIN_NAME].itervalues():
        raise InvalidAddress("Invalid address prefix: %.2X" % ord(addr_bytes[0]))


def validate_ddcscrd_address_bytes(addr_bytes):
...
def decode_address(v):
    ...
    return validate_ddcscrd_address_bytes(b58decode(v))


def encode_address(addr_bytes):
    """validate and encode an address as b58"""
    v = validate_ddcscrd_address_bytes(addr_bytes)
    return b58encode(v)


def hash_160_bytes_to_address(h160, addrtype=PUBKEY_ADDRESS):
    if addrtype == PUBKEY_ADDRESS:
        prefix = chr(ADDRESS_PREFIXES[ddcsschema.BLOCKCHAIN_NAME][PUBKEY_ADDRESS])
    elif addrtype == SCRIPT_ADDRESS:
        prefix = chr(ADDRESS_PREFIXES[ddcsschema.BLOCKCHAIN_NAME][SCRIPT_ADDRESS])
    else:
...
def address_to_hash_160(addr):
    ...
    if prefix == chr(ADDRESS_PREFIXES[ddcsschema.BLOCKCHAIN_NAME][PUBKEY_ADDRESS]):
